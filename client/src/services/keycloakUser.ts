...
// 입력 받을때 validate으로 이중관리 할 예정
export const onlyEnUsername = (username: string): string => {
  return username
    .replace(/[^\w.-]/g, '_') // 한글과 특수문자 언더스코어로 변경 정규식
    .toLowerCase();
};
...
export const createUserData = (
  ...
  const enUsername = onlyEnUsername(username);
...
export const createKeycloakUser = async ({
  ...
  const userData = createUserData(email, password, username);
